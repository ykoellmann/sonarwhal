...
/**
 * Stores the source file + line for each endpoint discovered by [RouteXGutterService].
 * Provides navigation from the tool window back to the C# controller method.
 *
 * Population happens in two ways:
 *  1. [RouteXGutterService] registers locations as it places gutter icons in open editors.
 *  2. A background project scan via [RouteXGutterService.scanProjectFilesInBackground] covers
        ...
        ProgressManager.getInstance().run(object : Modal(project, "Searching for endpoint source…", true) {
            override fun run(indicator: ProgressIndicator) {
                ApplicationManager.getApplication().runReadAction {
                    val gutterService = RouteXGutterService.getInstance(project)
